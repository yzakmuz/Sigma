...
class LessonRepository:
    ...
    def list(self, topic: str | None = None, level: str | None = None) -> list[Lesson]:
        """List all lessons, optionally filtered by topic and level."""
        lessons = list(self._lessons.values())

        if topic:
            lessons = [l for l in lessons if l.topic.value.lower() == topic.lower()]

        if level:
            lessons = [l for l in lessons if l.level.value.lower() == level.lower()]
